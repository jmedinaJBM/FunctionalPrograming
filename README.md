...
5.  **BiConsumer<T,U>:** Similar a *Consumer<T>*, con la diferencia de que este recibe dos parámetros, uno de tipo `T` y otro de tipo `U` No devuelve valor alguno. En Java se expresa así:  **`(T paramT, U paramU) ->`**  ;   sin el operador Lamda se vería así:  **`void accept(T paramT, U paramU)`**    Se utiliza en casos similares en donde se utiliza el *Consumer<T>*, donde se requiere hacer algún tipo de operación con los dos parámetros que entrega; en el ejemplo veremos como se implementa un Separador.
